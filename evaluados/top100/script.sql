...
-- 1. Crear una base de datos llamada películas. (1 Punto)
    ...
    -- conecta a base de datos peliculas
    \c peliculas 
...
-- 2. Cargar ambos archivos a su tabla correspondiente. (1 Punto)
    \COPY peliculas FROM 'download/peliculas.csv' csv header;
    \COPY reparto FROM 'download/reparto.csv' delimiter ',';
...
-- 4. Listar a todos los actores que aparecen en la película "Titanic". (1 Puntos)
    SELECT actor AS elenco_titanic FROM reparto WHERE id_pelicula = (SELECT id FROM peliculas WHERE pelicula = 'Titanic');

-- 5. Consultar en cuántas películas del top 100 participa Harrison Ford. (2 Puntos)
    SELECT COUNT(id_pelicula) AS top100_harrison_ford FROM reparto WHERE actor = 'Harrison Ford';


-- 6. Indicar las películas estrenadas entre los años 1990 y 1999 ordenadas por título de manera ascendente. (1 punto)

    SELECT pelicula AS estrenos_entre_1990_y_1999 FROM peliculas WHERE estreno BETWEEN 1990 AND 1999 ORDER BY pelicula ASC;
...
-- 8. Consultar cual es la longitud más grande entre todos los títulos de las películas. (2 punto)
    ...
    SELECT MAX(LENGTH(pelicula)) AS longuitud_maxima FROM peliculas;